...
import { fetchFile, fetchImageFile, fetchJpegFile, fetchPdfFile, fetchSvgFile, uploadFile } from "../../../../services/fileServices";
...
const ViewDocumentForm = ({ policyId, onClose }) => {
  ...
  const handleViewDocument = async (documentImage) => {
    try {
      ...
      } else if (documentImage.endsWith(".jpeg")) {
        url = await fetchJpegFile(documentImage);
      } else {
